...
- 按用户、项目和 Agent 灰度（部分完成，见下文"灰度分流总闸"：已有按项目/用户名单控制整个服务
  能不能进，尚未做到按名单决定能用哪些子 Agent/工具）；
- 只读模式开关（已完成，见下文）；
- Redis 依赖的生产强制校验（已完成，见下文；原计划外补充，属于"滚动发布不丢状态"的前置条件）；
- 优雅停机与发布安全（已完成，见下文；同样是原计划外补充）；
- 单用户和全局模型并发限制（已完成，见下文"模型调用熔断与并发限制"；进程内尽力而为限流，默认不限制）；
- 模型调用超时/重试预算与备用模型（已完成，见下文）；
- 会话锁续期看门狗（已完成，见下文；原计划外补充，属于"长对话不丢锁"的正确性前置条件）；
- 连续失败熔断（已完成，见下文"模型调用熔断与并发限制"；框架无 CircuitBreaker，自建在 `onModelCall`
  钩子上，默认开启）；
...
调研发现子 Agent 注册（`AgentCatalog`）、权限规则表（`PermissionContextState`）、工具目录
（`FrontendToolRegistrar`）这几个"能用哪些 Agent/工具"的关键装配点全部是**启动期固化的全局单例**，
不是按请求动态计算的。要做到"按名单决定这个用户/项目能用哪些子 Agent 或工具"，得把这几处都改造成
per-request 动态装配，成本和风险与阶段 9 评估过的"委派硬并发限制"是同一个量级——因此窄范围内先只做
最外层的总闸：**这个项目/用户能不能进 Agent 服务**，不区分进来之后能用什么。

判断点在 `AgentRunOrchestrator.run()`，紧跟在停机前置检查（`assertAcceptingRequests`）之后、
`prepareInput`（入参校验、会话归属检查）之前：比停机检查晚一步是因为停机是更紧急的拒绝信号，比后面
那些校验早一步是为了让名单外的请求尽快拿到明确反馈，而不是先看到一堆不相关的校验错误。

配置项 `agent.runtime.gray`：

```yaml
gray:
  enabled: true
  allowed-project-ids: [ "project-a", "project-b" ]
  allowed-user-ids: [ "internal-tester-1" ]
```

`enabled` 默认 `false`——不开灰度时对现有部署零影响，与 `read-only-mode`/`require-redis` 一样"限制性
功能默认关闭"。开启后，`allowed-project-ids` 与 `allowed-user-ids` 是 **OR 语义**：项目在名单里，或者
用户在名单里，任一命中就放行。以项目名单为主维度（`EffectiveAgentGray.isAllowed`）是因为 bk-repo 本来
就按项目隔离，是运营侧最自然的放量单位；用户名单是给内部测试账号跨项目验证用的豁免通道，不是第二个
主维度。

命中失败时抛 `PermissionException`（HTTP 403，`system.request.denied`），与会话归属校验失败用的是
同一个异常类型——语义上都是"这个请求不该被这个身份发起"，客户端不需要再新增一种错误处理分支。

**已知边界**：

- 只做总闸，不做按 Agent/工具的细粒度灰度——这是刻意的取舍，见上文。
- 名单是 `agent.runtime` 的一部分，改名单需要改配置中心 + 重启（或重建
  `EffectiveAgentRuntimeProperties` 这个 Bean），不是运行时热更新；`AgentRunOrchestrator` 里存的是
  单例 `properties` 的引用，热更新需要额外接一个能在不重启的前提下刷新配置的机制（比如 Spring Cloud
  的 `@RefreshScope`），本次没有引入。放量频率不高、按项目/批次推进的场景下，重启生效是可接受的
  代价；如果将来需要分钟级调整名单，再评估要不要为此单独接一套动态配置存储。
- 名单外的请求收到的是通用的"未开通"提示，不会告诉调用方具体是哪条名单没命中——避免把配置细节
  泄露给客户端。
...
框架本身没有熔断，重试预算耗尽只影响那一次调用；模型整体不可用（网关下线、配额耗尽、上游持续 5xx）时，
每个请求都会各自把重试预算走完再失败——上一节的预算收敛让"各自走完"变短了（约 3–6 分钟），但故障期间
每个请求依然要傻等这么久才能失败，白白占着并发名额和用户的等待时间。并发上限则是另一类问题：框架完全
没有限制模型调用并发的机制，理论上有多少个 run 同时在跑，就有多少个并发的模型调用。

两者都实现成 `MiddlewareBase.onModelCall` 钩子，而不是 `onReasoning` 或更外层：`onModelCall` 包住的
`next.apply(input)` 正是 `AgentModelConfig` 装配出来的那条调用链——框架内部的超时重试与单级 fallback
已经在 `next` 内部跑完，这一层拿到的成功/失败是"重试和 fallback 都试过之后"的最终结果，恰好是熔断该
关心的粒度；放在更外层会把工具执行失败也计入统计，语义就错了。

- **`ModelCircuitBreakerMiddleware`**：包一个三态熔断器（`ModelCircuitBreaker`，CLOSED → OPEN →
  HALF_OPEN）。CLOSED 态统计连续失败次数，达到 `agent.llm.circuit-breaker.failure-threshold`（默认 5）
  转 OPEN；OPEN 态直接拒绝、完全不调用 `next`——这是熔断最大的价值：省掉一整轮重试预算的等待，让调用方
  立刻知道"现在打不动"而不是干等几分钟；冷却（`cooldown`，默认 30s）结束后放一个探测请求过去并转
  HALF_OPEN，同一时刻只放行这一个探测（用 CAS 抢 OPEN → HALF_OPEN 的转换，抢不到的请求继续拒绝）；探测
  成功转回 CLOSED 并清零计数，失败或被取消（客户端断开、Agent 被打断，视为不确定结果，悲观按失败处理）
  转回 OPEN 重新计时。冷却期固定不做指数退避，是窄范围内的第一版：真出现反复"熔断-探测失败-再熔断"抖动
  再引入退避，现在加只是没被真实场景验证过的复杂度。
- **`ModelConcurrencyLimitMiddleware`**：包一个 `ModelConcurrencyGuard`，全局与按用户两个维度分别用
  `java.util.concurrent.Semaphore.tryAcquire()` 判断（非阻塞，打满直接拒绝，不排队等待——排队等待在
  reactive 调用链里没有意义，只会让请求看起来"卡住"而不是"被拒绝"），任一维度 ≤ 0 表示不限制该维度。
  与阶段 9 的 `DelegationConcurrencyGuard`（子代理委派并发）语义一致：都是进程内、非跨副本的尽力而为
  限流，不是需要强一致性的计费或配额系统——跨副本精确配额要靠 Redis 原子操作，为这个目的引入分布式协调
  不划算。

两者的默认值刻意不对称：**熔断默认开启**（`agent.llm.circuit-breaker.enabled=true`），因为它对健康流量
完全无感，只在真的连续失败时才触发，属于"默认开对用户无感、真出事时才有用"的安全网；**并发限制默认不
限制**（`max-global`/`max-per-user` 默认都是 0），因为它会主动拒绝本来健康的合法请求，是一个需要按环境
容量显式配置的限流阀，与 `read-only-mode`/`require-redis` 一样"限制性功能默认关闭"。

两个 Middleware 在 `AgentHarnessConfigurer` 里熔断注册在并发限制外层：熔断已经 OPEN 时应直接拒绝，不该
再去抢并发名额，也不该让并发限制的拒绝统计里混进"模型本来就在熔断"的噪音。拒绝时抛的
`ModelCircuitOpenException`/`ModelConcurrencyLimitExceededException` 没有特殊 HTTP 状态码映射，走与其它
模型调用失败相同的路径——`AgentRunEventPipeline` 按普通 run 失败收尾，`RunError` 事件的 `errorCode` 就是
异常类名，客户端可以据此区分"熔断中"与"并发已满"，用户重试即可，语义上都是"现在不行，等会再试"。

**已知边界**：熔断与并发限制都是单副本内的状态，不跨副本共享——熔断在多副本场景下需要每个副本各自攒够
连续失败次数才会分别跳闸，不是全局同步跳闸；并发上限也是"每副本各自的上限"，不是集群总量。这与
`DelegationConcurrencyGuard` 的设计取舍一致：本项目里没有一个副本会持有别的副本的会话（会话锁保证归属
唯一），所以单副本视角天然就是这个会话当前唯一活跃的视角，不需要跨副本协调。
...
- 按项目/用户控制谁能访问 Agent 服务（已完成：`agent.runtime.gray` 总闸，见上文"灰度分流总闸"；
  尚未做到按名单控制能用哪些子 Agent/工具）；
